...
namespace Tarsier.Config {
    public class SQLiteHelper {
        ...
        public SQLiteHelper(string filename) {
            if(!this.IsDatabaseExist(filename)) {
                SQLiteConnection.CreateFile(filename);
            }
            this.con = new SQLiteConnection(string.Format("Data Source={0};Version=3;New=False;Compress=True;", filename));
            this.con.Open();
            this.cmd = this.con.CreateCommand();
        }

        private bool IsDatabaseExist(string filename) {
        ...
        public bool IsTableExist(string tableName) {
            ...
            if(dt != null) {
                foreach(DataRow dr in dt.Rows) {
                    if(dr[0].ToSafeString().Equals(tableName.Trim())) {
                        return true;
                    }
                }
            }
        ...
        public void AddColumn(SQLiteTable table) {
            foreach(SQLiteColumn col in table.Columns) {
                if(col.ColumnName.Trim().Length == 0) {
                    throw new Exception("Column name cannot be blank.");
                }
                ...
                switch(col.ColDataType) {
                    case ColType.Text:
                        columnType = "text"; break;
                    case ColType.Integer:
                        columnType = "integer"; break;
                    case ColType.Decimal:
                        columnType = "decimal"; break;
                    case ColType.DateTime:
                        columnType = "datetime"; break;
                    case ColType.BLOB:
                        columnType = "blob"; break;
                }
                AddColumn(table.GetCurrentTable, col.ColumnName.Trim(), columnType);
            }
        }

        public void AddColumn(string table, Dictionary<string, string> columns) {
            if(columns.Count > 0) {
        ...
        public void AddColumn(string table, string columnName, string columnType) {
            cmd.CommandText = string.Format("ALTER TABLE `{0}` ADD COLUMN {1} {2};", table, columnName, columnType);
            cmd.ExecuteNonQuery();
        }
        ...
        public bool IsExist(string tablename, string columname, string value) {
            string query = String.Format("select * from {0} where {1} = {2}", tablename, columname, value);
        ...
        public string GetLastID(string tableName, string primaryField = "id") {
            string sql = string.Format("select {0} from {1}  order by {0} desc limit 1;", primaryField, tableName);
            DataTable dt = Select(sql);
            foreach(DataRow dr in dt.Rows) {
                return dr[primaryField].ToSafeString();
            }
            return string.Empty;
        }
        public string GetColumnValue(string tableName, string columnName, string whereCondition) {
            string sql = string.Format("select {0} from {1} where {2} order by {0} desc limit 1;", columnName, tableName, whereCondition);
            DataTable dt = Select(sql);
            foreach(DataRow dr in dt.Rows) {
                return dr[columnName].ToSafeString();
            }
            return string.Empty;
        }
